...
class TestTimeseriesByCampaignsApi:
    ...
    def test_timeseries_by_campaigns_as_admin_api(
...
class TestTimeseriesByCampaignForCampaignApi:
    ...
    def test_timeseries_by_campaign_for_campaign_api(
        self, app, user, users, campaigns, timeseries_by_campaigns
    ):
        campaign_1_id, _ = campaigns
        tbc_1_id, _ = timeseries_by_campaigns
        ...
        with auth_context:
            ...
            # GET list
            ret = client.get(
                f"{CAMPAIGNS_URL}{campaign_1_id}/timeseriesbycampaigns/")
            if user == "anonym":
            ...
            ret = client.get(
                ...
                f"timeseriesbycampaigns/{tbc_1_id}"
            )
            if user == "anonym":
                assert ret.status_code == 401
            else:
                assert ret.status_code == 200
                assert ret.json["id"] == tbc_1_id
