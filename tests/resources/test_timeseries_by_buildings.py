...
class TestTimeseriesByBuildingApi:
    def test_timeseries_by_buildings_api(self, app, users, buildings, timeseries):
        ...
        with AuthHeader(creds):
            ...
            # DELETE wrong ID -> 404
            ret = client.delete(f"{TIMESERIES_BY_BUILDINGS_URL}{DUMMY_ID}")
            ...
            # DELETE
            ret = client.delete(f"{TIMESERIES_BY_BUILDINGS_URL}{tbb_1_id}")
            assert ret.status_code == 404
            ret = client.delete(f"{TIMESERIES_BY_BUILDINGS_URL}{tbb_2_id}")
            assert ret.status_code == 204
