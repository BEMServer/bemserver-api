...
class TestEventBySpaceApi:
    def test_events_by_spaces_api(self, app, users, spaces, events):
        ...
        with AuthHeader(creds):
            ...
            # PUT event + space from different campaigns
            ebs_1["space_id"] = space_2_id
            ret = client.put(
                f"{EVENTS_BY_SPACES_URL}{ebs_1_id}",
                json=ebs_1,
                headers={"If-Match": ebs_1_etag},
            ...
            # PUT wrong ID -> 404
            ret = client.put(
                f"{EVENTS_BY_SPACES_URL}{DUMMY_ID}",
                json=ebs_1,
                headers={"If-Match": ebs_1_etag},
            )
            ...
            assert ret.status_code == 404
            ret = client.delete(
                f"{EVENTS_BY_SPACES_URL}{ebs_2_id}",
    ...
    def test_events_by_spaces_as_user_api(
        ...
        with AuthHeader(user_creds):
            ...
            # PUT
            ebs_3["space_id"] = space_1_id
            ret = client.put(
                f"{EVENTS_BY_SPACES_URL}{ebs_3_id}",
                json=ebs_3,
