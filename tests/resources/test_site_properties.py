...
class TestSitePropertiesApi:
    def test_site_properties_api(self, app, users, structural_element_properties):
        ...
        with AuthHeader(creds):
            ...
            site_p_2_etag = ret.headers["ETag"]
            ...
            # DELETE wrong ID -> 404
            ret = client.delete(
                f"{SITE_PROPERTIES_URL}{DUMMY_ID}",
                headers={"If-Match": site_p_1_etag},
            )
            assert ret.status_code == 404

            # DELETE
            ret = client.delete(
                f"{SITE_PROPERTIES_URL}{site_p_1_id}",
                headers={"If-Match": site_p_1_etag},
            )
            assert ret.status_code == 204
            ret = client.delete(
                f"{SITE_PROPERTIES_URL}{site_p_2_id}",
                headers={"If-Match": site_p_2_etag},
            )
            assert ret.status_code == 204
