...
TIMESERIES_EVENTS_SUB_URL = f"{EVENTS_URL}timeseries/"


class TestTimeseriesEventsApi:
    def test_timeseries_events_api(self, app, users, event_channels):
        ...
        with AuthHeader(creds):
            ...
            # GET list
            ret = client.get(TIMESERIES_EVENTS_SUB_URL)
            ...
            ret = client.post(TIMESERIES_EVENTS_SUB_URL, json=tse_1)
            ...
            # GET by id
            ret = client.get(f"{TIMESERIES_EVENTS_SUB_URL}{tse_1_id}")
            assert ret.status_code == 200
            ...
            ret = client.post(TIMESERIES_EVENTS_SUB_URL, json=tse_2)
            assert ret.status_code == 201
            ret_val = ret.json
            ...
            # GET list (filtered)
            ret = client.get(
                TIMESERIES_EVENTS_SUB_URL, query_string={"channel_id": ec_1_id}
            )
            assert ret.status_code == 200
            ret_val = ret.json
            assert len(ret_val) == 1
            assert ret_val[0]["id"] == tse_1_id
            ret = client.get(
                TIMESERIES_EVENTS_SUB_URL, query_string={"level": "WARNING"}
            )
            assert ret.status_code == 200
            ret_val = ret.json
            assert len(ret_val) == 1
            ...
            ret = client.delete(
                f"{TIMESERIES_EVENTS_SUB_URL}{DUMMY_ID}",
            ...
            # DELETE
            ret = client.delete(
                f"{TIMESERIES_EVENTS_SUB_URL}{tse_1_id}",
                headers={"If-Match": tse_1_etag},
            )
            assert ret.status_code == 204
            ret = client.delete(
                f"{TIMESERIES_EVENTS_SUB_URL}{tse_2_id}",
    ...
    def test_timeseries_events_as_user_api(
        self, app, users, event_channels, campaigns, timeseries_events
    ):
        tse_1_id = timeseries_events[0]
        event_channel_1_id = event_channels[0]
        ...
        with AuthHeader(creds):
            ...
            ret = client.post(TIMESERIES_EVENTS_SUB_URL, json=tse)
            assert ret.status_code == 201
    ...
    def test_timeseries_events_as_anonym_api(
        self, app, event_channels, timeseries_events
    ):
        tse_1_id = timeseries_events[0]
        event_channel_1_id = event_channels[0]
        c1_st = dt.datetime(2021, 1, 1, tzinfo=dt.timezone.utc).isoformat()
